...
export function Navbar() {
  ...
  const headerLinks = [
    ...
    { label: t('nav.about'), href: '#teacher-problem' },
    { label: t('nav.features'), href: '#المميزات' },
    { label: t('nav.howItWorks'), href: '#how-it-works' },
    { label: t('nav.contact'), href: '#تواصل-معنا' },
  ]
    ...
    <header className="sticky top-0 z-50 h-[76px] border-b border-[#E5E7EB] bg-white/95 font-['Cairo'] backdrop-blur-xl">
      <Container className="h-full px-6">
        <div className="relative flex h-full items-center justify-between gap-6">
          <div className="order-3 hidden shrink-0 items-center gap-3 xl:flex" aria-label={t('menu.account')}>
            <LanguageSwitcher />
            <Link to="/login" className="inline-flex h-11 items-center justify-center rounded-xl border border-[#059669] bg-white px-6 text-sm font-semibold text-[#059669] transition-all duration-200 hover:bg-[#059669] hover:text-white hover:shadow-md hover:shadow-emerald-900/15 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-4">{t('auth.signIn')}</Link>
          </div>

          <nav className="order-2 hidden min-w-0 xl:flex xl:flex-1 xl:justify-center" aria-label={t('menu.navigation')}>
          ...
          <button onClick={() => setIsOpen(open => !open)} className="order-2 ms-auto rounded-xl p-2.5 text-slate-700 transition hover:bg-emerald-50 hover:text-emerald-700 focus:outline-none focus:ring-2 focus:ring-emerald-600 focus:ring-offset-4 xl:hidden" aria-label={isOpen ? t('menu.close') : t('menu.open')} aria-expanded={isOpen} aria-controls="mobile-navigation">
      ...
      <AnimatePresence>
        {isOpen && <motion.nav id="mobile-navigation" aria-label={t('menu.mobileNavigation')} initial={{ opacity: 0, y: -12 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: -12 }} transition={{ duration: 0.24, ease: 'easeOut' }} className="absolute inset-x-0 top-full w-full border-b border-slate-100 bg-white p-4 shadow-xl shadow-slate-900/10 sm:start-auto sm:end-4 sm:max-w-sm sm:rounded-b-2xl sm:border sm:p-5 xl:hidden">
          ...
          <div className="mt-5 space-y-3 border-t border-slate-100 pt-5"><LanguageSwitcher /><Link to="/login" onClick={() => setIsOpen(false)} className="flex h-11 items-center justify-center rounded-xl border border-[#059669] bg-white px-6 text-sm font-semibold text-[#059669] transition-all duration-200 hover:bg-[#059669] hover:text-white hover:shadow-md hover:shadow-emerald-900/15">{t('auth.signIn')}</Link></div>
        </motion.nav>}
      </AnimatePresence>
    </header>
  )
